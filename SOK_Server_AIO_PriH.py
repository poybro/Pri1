#!/usr/bin/env python3
# SOK_Server_All_In_One - v15.0_Hardened (Bản vá Bảo mật & Tối ưu Config)
# -*- coding: utf-8 -*-
...
# --- CẤU HÌNH CHUNG ---
# [ĐÃ SỬA] Bỏ cấu hình từ biến môi trường, đặt giá trị trực tiếp.
PRIME_WALLET_FILE = "prime_agent_wallet.pem"; STATE_FILE = "prime_agent_state.json"; SERVER_PORT = 9000
VALID_ACTIVATION_KEY = "SOK@10" # Đặt key kích hoạt cố định tại đây
...
CONFIG_CACHE_SECONDS = 300 # [TỐI ƯU HÓA] Dữ liệu config từ GitHub sẽ được cache trong 5 phút
...
class PrimeAgentLogic:
    ...
    def get_config(self, filename: str) -> Optional[Dict]:
        with self.config_cache_lock:
            cache_entry = self.config_cache.get(filename)
            if cache_entry and (time.time() - cache_entry['timestamp']) < CONFIG_CACHE_SECONDS:
                logging.debug(f"CONFIG_CACHE: Lấy '{filename}' từ cache.")
                return cache_entry['data']
        
        config_data = self._fetch_config_from_source(filename)
        if config_data:
            with self.config_cache_lock:
                self.config_cache[filename] = {'data': config_data, 'timestamp': time.time()}
        return config_data

    def _fetch_config_from_source(self, filename: str) -> Optional[Dict]:
        url = GITHUB_CONFIG_BASE_URL + filename
        logging.info(f"CONFIG_FETCH: Đang tải file cấu hình từ GitHub: {url}")
        try:
            response = requests.get(url, timeout=20)
            response.raise_for_status()
            config_data = response.json()
            logging.info(f"CONFIG_FETCH: Tải thành công '{filename}' từ GitHub.")
            try:
                with open(filename, 'w', encoding='utf-8') as f: json.dump(config_data, f, indent=2)
            except Exception as e:
                logging.warning(f"CONFIG_FETCH: Không thể lưu cache cục bộ cho '{filename}': {e}")
            return config_data
        except (requests.RequestException, json.JSONDecodeError) as e:
            logging.warning(f"CONFIG_FETCH: Không thể tải hoặc phân tích file từ GitHub: {e}. Thử tải từ file cục bộ...")
            if os.path.exists(filename):
                try:
                    with open(filename, 'r', encoding='utf-8') as f:
                        logging.info(f"CONFIG_FETCH: Tải thành công '{filename}' từ file cục bộ (fallback).")
                        return json.load(f)
                except Exception as e_local:
                    logging.error(f"CONFIG_FETCH: Lỗi khi đọc file cục bộ '{filename}': {e_local}")
                    return None
            else:
                logging.error(f"CONFIG_FETCH: Cả GitHub và file cục bộ cho '{filename}' đều không khả dụng.")
                return None
    ...
    # [ĐÃ SỬA] Đơn giản hóa hàm, luôn yêu cầu nhập mật khẩu khi khởi động.
    def _load_admin_config(self) -> str:
        ...
        if not password:
            logging.critical("❌ Mật khẩu không được để trống.")
            sys.exit(1)
        return password
    ...
    def _load_auto_auth_config(self):
        ...
        config = self.get_config(FEDERATION_PEERS_FILE)
        if not config:
        ...
        with self.state_lock:
            ...
            try:
                ...
                if not os.path.exists(script_path):
                    logging.error(f"MINER_CONTROL: Không tìm thấy file miner tại '{script_path}'.")
                    return {"error": f"Không tìm thấy file '{'sok_intelligent_miner.py'}'."}, 500
                
                if not script_path.endswith('.py'):
                     return {"error": "Đường dẫn file miner không hợp lệ."}, 500
                ...
                logging.error(f"MINER_CONTROL: Lỗi khi bắt đầu thợ mỏ: {e}", exc_info=True)
    ...
    def _load_state(self):
        if os.path.exists(STATE_FILE):
            try:
                ...
                if not isinstance(self.active_workers, dict): self.active_workers = {}

                self.last_reward_times = state.get("last_reward_times", {})
                if not isinstance(self.last_reward_times, dict): self.last_reward_times = {}

                raw_db = state.get("websites_db", {})
                self.websites_db = {}
                if isinstance(raw_db, dict):
                    for url, data in raw_db.items():
                        if isinstance(data, dict) and isinstance(url, str):
                            self.websites_db[url] = {
                                'owner': data.get('owner'),
                                'views_funded': Decimal(str(data.get('views_funded', '0'))),
                                'views_completed': Decimal(str(data.get('views_completed', '0')))
                            }
                
                self.last_scanned_block = int(state.get("last_scanned_block", -1))
                ...
                if isinstance(raw_pending, dict):
                    for addr, amt_str in raw_pending.items():
                        if isinstance(addr, str): self.pending_funds[addr] = Decimal(str(amt_str))

                logging.info(f"Đã khôi phục và xác thực trạng thái: {len(self.active_workers)} worker, {len(self.websites_db)} website.")

            except (json.JSONDecodeError, TypeError, ValueError) as e:
                logging.error(f"Không thể tải hoặc xác thực tệp trạng thái bị lỗi: {e}. Bắt đầu với trạng thái mới.")
            except Exception as e:
                logging.error(f"Lỗi không xác định khi tải trạng thái: {e}")
    ...
    def p2p_create_order(self, seller_address, sok_amount_str, fiat_details):
        ...
        with self.state_lock:
            ...
            if time.time() - last_create_time < 180:
                remaining_time = 180 - int(time.time() - last_create_time)
                return {"error": f"Bạn cần đợi {remaining_time} giây nữa để tạo lệnh bán mới."}, 429
    ...
    def _econ_generate_chart(self):
        try:
            ...
            fig.update_layout(
                title_text='<b>Mô hình Định giá & Sức khỏe Mạng lưới Sokchain</b>',
                yaxis=dict(
                    title='<b>Giá SOK (USD)</b>',
                    type='log',
                    gridcolor='rgba(255, 255, 255, 0.1)',
                    tickformat=".8f"
                ),
                yaxis2=dict(
                    title='Giá trị Quỹ (USD)',
                    overlaying='y',
                    side='right',
                    showgrid=False
                ),
            ...
            if not self.historical_econ_data:
                fig.add_annotation(
                    text="Đang chờ dữ liệu kinh tế đầu tiên...",
                ...
                chart_mode = 'markers' if len(data_copy) == 1 else 'lines+markers'

                fig.add_trace(go.Scatter(
                    x=timestamps, y=floor_prices, mode=chart_mode, name='Giá Sàn (Bảo chứng)',
                    line=dict(color='#00FF7F', dash='dot', width=2),
                    fill='tozeroy', fillcolor='rgba(0, 255, 127, 0.1)'
                ))
                fig.add_trace(go.Scatter(
                    x=timestamps, y=market_prices, mode=chart_mode, name='Giá Thị trường (Ước tính)',
                    line=dict(color='#00BFFF', width=3),
                    marker=dict(size=8 if len(data_copy) == 1 else 5, symbol='circle-open')
                ))
                fig.add_trace(go.Bar(
                    x=timestamps, y=treasury_values, name='Tổng Quỹ Bảo chứng (USD)',
                    yaxis='y2', marker_color='#FFD700', opacity=0.5
                ))
            
            fig.write_html(ECON_CHART_FILE, full_html=False, include_plotlyjs='cdn', config={'responsive': True})
            logging.info(f"Agent Kinh tế: ✅ Biểu đồ (Responsive, Transparent) đã được cập nhật.")
        except Exception as e:
            logging.error(f"Lỗi khi vẽ biểu đồ kinh tế: {e}", exc_info=True)
    ...
    def _econ_run_cycle(self):
        ...
        if last_analysis:
            try:
                ...
                avg_fee_per_tx = PRICE_PER_VIEW * 10 * (PLATFORM_FEE_PERCENT / 100)
                revenue_sok = Decimal(str(new_transactions)) * avg_fee_per_tx
                revenue_usd = revenue_sok * last_market_price
                with self.state_lock:
                    self.treasury_value_usd += revenue_usd
            except Exception as e:
                logging.error(f"ECON_CYCLE: Lỗi khi tính doanh thu: {e}")

        with self.state_lock:
            current_treasury_usd = self.treasury_value_usd
        total_supply = ECON_INITIAL_TOTAL_SUPPLY

        # --- 2. Tính Giá Sàn ---
        floor_price_usd = current_treasury_usd / total_supply

        # --- 3. Tính Hệ số Hoạt động (Activity Multiplier) ---
        activity_multiplier = Decimal('0.0')
        if last_analysis:
            def calculate_growth(current, last):
                if last > 0:
                    return Decimal(str(current - last)) / Decimal(str(last))
                return Decimal('0.0') if current == 0 else Decimal('1.0')

            tx_growth = calculate_growth(current_metrics['total_transactions'], last_analysis.get('total_transactions', 0))
            worker_growth = calculate_growth(current_metrics['total_workers'], last_analysis.get('total_workers', 0))
            website_growth = calculate_growth(current_metrics['total_websites'], last_analysis.get('total_websites', 0))

            CAPPED_GROWTH = Decimal('2.0')
            tx_growth = min(tx_growth, CAPPED_GROWTH)
            worker_growth = min(worker_growth, CAPPED_GROWTH)
            website_growth = min(website_growth, CAPPED_GROWTH)

            def smooth_growth(growth_rate):
                if growth_rate >= 0:
                    return Decimal(str(math.log1p(growth_rate)))
                else:
                    return -Decimal(str(math.log1p(abs(growth_rate))))

            smoothed_tx = smooth_growth(tx_growth)
            smoothed_worker = smooth_growth(worker_growth)
            smoothed_website = smooth_growth(website_growth)
            
            activity_multiplier = (ECON_W_TX_GROWTH * smoothed_tx) + \
                                  (ECON_W_WORKER_GROWTH * smoothed_worker) + \
                                  (ECON_W_WEBSITE_GROWTH * smoothed_website)
        
        # --- 4. Tính Giá Thị trường Ước tính ---
        market_price_usd = floor_price_usd * (Decimal('1.0') + activity_multiplier)
        market_price_usd = max(Decimal('0.0'), market_price_usd)
        ...
        analysis_result = {
            **current_metrics,
            ...
            "treasury_value_usd": current_treasury_usd
        }
        self.historical_econ_data.append(analysis_result)
        self._econ_save_data()
        self._econ_generate_chart()

    # [ĐÃ SỬA] Khắc phục lỗi thụt lề và lỗi logic vòng lặp để xử lý log.
    def _econ_cycle_loop(self):
        self._econ_load_data()
        # Chờ một chút để các luồng khác ổn định
        time.sleep(15)
        while self.is_running.is_set():
            try:
                ...
                # Dùng time.sleep() để đảm bảo luồng tạm dừng đúng_khoảng thời gian
...
def admin_authenticated(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if not auth_pass or auth_pass != core_logic.admin_password:
            return jsonify({"error": "Không có quyền truy cập hoặc mật khẩu không đúng."}), 403
        return f(*args, **kwargs)
    return decorated_function
...
def add_website():
    ...
    with core_logic.state_lock:
        ...
        if time.time() - last_add_time < 180:
            remaining_time = 180 - int(time.time() - last_add_time)
            return jsonify({"error": f"Bạn cần đợi {remaining_time} giây nữa để thêm website mới."}), 429
...
if __name__ == '__main__':
    ...
    try:
        ...
        print(Fore.GREEN + Style.BRIGHT + f"   ΣOK CHAIN - AIO SERVER v15.0_Hardened (Port {SERVER_PORT})")
